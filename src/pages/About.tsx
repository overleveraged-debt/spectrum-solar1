...
import { Award, Shield, Users, Zap, Clock, Globe, ArrowRight, Quote } from 'lucide-react';
...
const DEFAULT_ABOUT_DATA = {
  ...
  presenceDesc: 'Explore our extensive network of 18+ service centers and thousands of installations across the state.',
  ...
  ceoMessage: 'For over two decades, our mission at Spectrum Powers has been simple yet ambitious: to democratize clean, reliable, and affordable solar energy across India. By combining world-class engineering with dedicated customer support, we continue to empower homes, businesses, and communities to transition towards complete energy independence.',
...
const About: React.FC = () => {
  useScrollReveal();
  const [pageData, setPageData] = useState(DEFAULT_ABOUT_DATA);

  useEffect(() => {
    let isMounted = true;
    sanityReadClient.fetch('*[_type == "pageContent" && pageId == "about"][0]')
      .then(res => {
  ...
  const stats = [
    { label: pageData.stat1Label, value: pageData.stat1Value, icon: <Clock className="w-6 h-6" />, sub: pageData.stat1Sub },
    { label: pageData.stat2Label, value: pageData.stat2Value, icon: <Users className="w-6 h-6" />, sub: pageData.stat2Sub },
    { label: pageData.stat3Label, value: pageData.stat3Value, icon: <Zap className="w-6 h-6" />, sub: pageData.stat3Sub },
    { label: pageData.stat4Label, value: pageData.stat4Value, icon: <Shield className="w-6 h-6" />, sub: pageData.stat4Sub },
  ];
  ...
  return (
    <div className="bg-zinc-950 text-white pb-20 overflow-x-hidden">
        ...
        <section className="relative h-[80vh] flex items-center justify-center overflow-hidden pt-24 mt-[-80px]">
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-6 text-center">
            ...
            <h1 className="text-[1.9rem] sm:text-5xl md:text-6xl lg:text-7xl font-thin tracking-tight mb-6 leading-[0.9] uppercase text-white drop-shadow-[0_10px_35px_rgba(0,0,0,0.6)]">
              {pageData.heroTitle.includes('25 YEARS OF') ? (
                <>25 YEARS OF <br className="hidden md:block" /> ENERGY EXCELLENCE</>
              ) : (
                pageData.heroTitle
              )}
            </h1>
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            <div className="reveal">
              ...
              <div className="flex items-center gap-6">
                <div>
                  <p className="text-white font-black text-2xl mb-0.5">{pageData.heritageFounded}</p>
                  <p className="text-zinc-600 text-[10px] font-black uppercase tracking-widest">Founded</p>
                </div>
                <div className="w-px h-10 bg-white/10"></div>
                <div>
                  <p className="text-white font-black text-2xl mb-0.5">{pageData.heritageHeadquarters}</p>
                  <p className="text-zinc-600 text-[10px] font-black uppercase tracking-widest">Headquarters</p>
                </div>
                <div className="w-px h-10 bg-white/10"></div>
                <div>
                  <p className="text-white font-black text-2xl mb-0.5">{pageData.heritageCenters}</p>
      ...
      {/* Quality Section */}
      {pageData.showQuality !== false && (
        <section className="px-6 py-24 border-t border-white/5">
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            <div className="reveal border border-white/5 overflow-hidden rounded-[2.5rem] aspect-[4/3] shadow-2xl order-2 md:order-1">
              <img
                src={pageData.qualityImage}
            ...
            <div className="reveal order-1 md:order-2" style={{ transitionDelay: '150ms' }}>
              ...
              <div className="p-6 bg-yellow-400/10 border border-yellow-400/20 rounded-2xl">
                ...
                <p className="text-white font-black uppercase tracking-tight">{pageData.qualityAwardTitle}</p>
                <p className="text-zinc-400 text-[10px] mt-1">{pageData.qualityAwardDesc}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-24 border-t border-white/5 bg-zinc-950">
          <div className="max-w-7xl mx-auto px-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6 reveal">
              <div>
                <span className="text-yellow-400 font-medium text-[10px] uppercase tracking-[0.5em] mb-4 block">{pageData.presenceSubtitle}</span>
                <h2 className="text-4xl md:text-7xl font-thin uppercase tracking-[-0.04em] leading-[0.9] text-white">
                  {pageData.presenceTitle === 'Powering Every District.' ? (
                    <>Powering Every <br />District.</>
                  ) : (
                    pageData.presenceTitle
                  )}
                </h2>
              </div>
              <p className="text-zinc-500 text-sm max-w-xs leading-relaxed md:text-right">
                {pageData.presenceDesc}
              </p>
            </div>
            <div className="reveal">
        ...
        <section className="px-6 pb-10">
          <div className="max-w-7xl mx-auto bg-yellow-400 text-black rounded-[2.5rem] md:rounded-[3rem] p-12 md:p-16 flex flex-col md:flex-row items-center justify-between gap-8 reveal">
            <div className="text-center md:text-left">
              <h2 className="text-4xl md:text-6xl font-thin tracking-tight leading-none uppercase mb-4">
                {pageData.ctaTitle === '18+ Centers Across India.' ? (
