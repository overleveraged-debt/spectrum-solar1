...
import {
  Zap, BatteryCharging, ArrowRight, ShieldCheck, CheckCircle2,
...
const Inverters: React.FC = () => {
  ...
  const { pageData } = usePageContent('inverters');
  const perfectList = pageData.perfectFor || [
    {
        "label": "Independent Houses",
        "sub": "Basic home backup protection"
    },
    {
        "label": "Offices & Workspaces",
        "sub": "Keep essential computers running"
    },
    {
        "label": "Residential Apartments",
        "sub": "Compact backup systems for everyday use"
    },
    {
        "label": "Retail Shops & Boutiques",
        "sub": "Billing terminals and display lighting"
    }
];
    ...
    <div className="bg-zinc-950 text-white min-h-screen overflow-x-hidden">
          ...
          <div className="relative z-10 w-full bg-yellow-400 mt-auto flex-shrink-0">
            <div className="max-w-7xl mx-auto px-6">
              <div className="grid grid-cols-2 md:grid-cols-4">
                {[
                  { value: pageData.stat1Value || "Pure", label: pageData.stat1Label || "Sine Wave Output" },
                  { value: pageData.stat2Value || "100%", label: pageData.stat2Label || "Appliance Safe" },
                  { value: pageData.stat3Value || "Silent", label: pageData.stat3Label || "Operation" },
                  { value: pageData.stat4Value || "Smart", label: pageData.stat4Label || "Battery Charging" },
                ].map((stat, i) => (
      ...
      <section className="py-24 md:py-32 px-6">
        <div className="max-w-3xl mx-auto">
          ...
          {
                    "title": "Load Analysis",
                    "desc": "Assessing home/office load capacity to select correct inverter VA rating."
          },
          {
                    "title": "Battery Allocation",
                    "desc": "Sizing external battery bank based on required backup time."
          },
          {
                    "title": "Inverter Hookup",
                    "desc": "Clean wall/trolley setup and primary electrical connection."
          },
          {
                    "title": "Multi-Stage Charge Setup",
                    "desc": "Configuring battery charging profiles to optimize lifetime."
          },
          {
                    "title": "Safety Run Test",
                    "desc": "Testing short-circuit cuts, load transitions, and overload trips."
          }
