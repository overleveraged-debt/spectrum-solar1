...
const resolveIcon = (id: string) => {
  switch (id) {
    ...
    case 'lithium-batteries':
      return <Battery className="w-5 h-5 md:w-7 md:h-7" />;
    case 'home-ups':
      return <Zap className="w-5 h-5 md:w-7 md:h-7" />;
    case 'inverters':
    case 'tubular-batteries':
      return <BatteryCharging className="w-5 h-5 md:w-7 md:h-7" />;
    case 'online-ups':
      return <Server className="w-5 h-5 md:w-7 md:h-7" />;
    default:
      return <Battery className="w-5 h-5 md:w-7 md:h-7" />;
  }
};
...
const PowerBackup: React.FC = () => {
  ...
  const [pageData, setPageData] = useState(DEFAULT_BACKUP_DATA);

  useEffect(() => {
    let isMounted = true;
    sanityReadClient.fetch('*[_type == "pageContent" && pageId == "power-backup"][0]')
      .then(res => {
        if (isMounted && res && res.content) {
          try {
            ...
            console.error("Failed to parse power-backup content", e);
          }
        }
      })
      .catch(err => console.error("Error fetching power-backup content:", err));
    return () => {
      isMounted = false;
    };
  }, []);
  ...
  const activeProductsList = pageData.products || [];

  const categorySchema = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    "itemListElement": activeProductsList.map((p, index) => ({
      "@type": "ListItem",
      "position": index + 1,
      "item": {
        "@type": "Product",
  ...
  return (
    <div className="bg-zinc-950 text-white pb-20 overflow-x-hidden min-h-screen">
        ...
        <section className="relative h-[80vh] flex items-center justify-center overflow-hidden pt-24 mt-[-80px]">
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-6 text-center">
            <h1 className="text-[1.9rem] sm:text-5xl md:text-6xl lg:text-7xl font-thin tracking-tight mb-6 leading-[0.9] uppercase text-white drop-shadow-[0_10px_35px_rgba(0,0,0,0.8)]">
              {pageData.heroTitle.includes('Power Backup') ? (
                <>Power Backup <br className="hidden md:block" /> Solutions</>
              ) : (
                pageData.heroTitle
              )}
            </h1>
      ...
      {/* Product Catalog */}
      {pageData.showProducts !== false && (
        <section className="pt-24 pb-32 bg-zinc-950">
          <div className="max-w-7xl mx-auto px-6">
            <div className="space-y-4">
              {activeProductsList.map((product, index) => {
                const accentColor = product.accentColor || '#facc15';
                return (
                  <div key={product.id} id={product.id} className="reveal group" style={{ transitionDelay: `${index * 80}ms` }}>
                    ...
                    >
                      <div
                        ...
                        style={{ backgroundColor: activeProduct === product.id ? accentColor : 'transparent' }}
                      />
                      ...
                      <div className="flex items-center gap-3 md:gap-10 p-4 md:p-8 pl-6 md:pl-10">
                        <span
                          ...
                          style={{ color: activeProduct === product.id ? accentColor : '#52525b' }}
                        >
                        ...
                        <div
                          ...
                          style={{
                            backgroundColor: activeProduct === product.id ? accentColor + '20' : 'rgba(255,255,255,0.04)',
                            color: activeProduct === product.id ? accentColor : '#71717a',
                            border: `1px solid ${activeProduct === product.id ? accentColor + '40' : 'rgba(255,255,255,0.06)'}`,
                          }}
                        >
                          {resolveIcon(product.id)}
                        </div>
                        ...
                        <div
                          ...
                          style={{
                            borderColor: activeProduct === product.id ? accentColor + '60' : 'rgba(255,255,255,0.1)',
                            color: activeProduct === product.id ? accentColor : '#52525b',
      ...
      <section className="px-6 pb-10">
        <div className="max-w-7xl mx-auto border border-yellow-400/20 bg-zinc-900 text-white rounded-[2.5rem] md:rounded-[3rem] p-6 sm:p-10 md:p-16 flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12 reveal shadow-2xl">
          <div className="text-center md:text-left">
            <h2 className="text-3xl sm:text-5xl md:text-6xl font-thin uppercase tracking-tight leading-none mb-4">
              Never Lose Power Again.
            </h2>
            <p className="font-medium text-zinc-500 max-w-md mx-auto md:mx-0 text-xs sm:text-sm md:text-base">
