...
const DEFAULT_PRODUCTS: Product[] = [
  ...
  },
  {
    ...
    description:
      'High-performance solar water heaters utilizing evacuated tube collectors (ETC) or flat plate collectors (FPC). Ideal for residential bathrooms, hotels, and industrial washing requirements, drastically reducing monthly electricity bills.',
    accentColor: '#facc15',
    features: [
      'ETC & FPC Options',
...
const resolveIcon = (id: string) => {
  switch (id) {
    case 'on-grid':
      return <Sun className="w-5 h-5 md:w-7 md:h-7" />;
    case 'hybrid':
      return <Zap className="w-5 h-5 md:w-7 md:h-7" />;
    case 'off-grid':
      return <Leaf className="w-5 h-5 md:w-7 md:h-7" />;
    case 'water-heaters':
      return <Thermometer className="w-5 h-5 md:w-7 md:h-7" />;
    default:
      return <Sun className="w-5 h-5 md:w-7 md:h-7" />;
  }
};
...
const SolarSolutions: React.FC = () => {
  ...
  const [pageData, setPageData] = useState(DEFAULT_SOLAR_DATA);

  useEffect(() => {
    let isMounted = true;
    sanityReadClient.fetch('*[_type == "pageContent" && pageId == "solar-solutions"][0]')
      .then(res => {
        if (isMounted && res && res.content) {
          try {
            ...
            console.error("Failed to parse solar-solutions page content", e);
          }
        }
      })
      .catch(err => console.error("Error fetching solar-solutions page content:", err));
    return () => {
      isMounted = false;
    };
  }, []);
        ...
        <section className="relative h-[80vh] flex items-center justify-center overflow-hidden pt-24 mt-[-80px]">
          <div className="absolute inset-0 z-0">
            <img
              ...
              alt="Spectrum Solar Panel installations"
            />
        ...
        <div className="max-w-7xl mx-auto border border-yellow-400/20 bg-zinc-900 text-white rounded-[2.5rem] md:rounded-[3rem] p-10 md:p-16 flex flex-col md:flex-row items-center justify-between gap-12 reveal shadow-2xl">
          <div className="text-center md:text-left">
            ...
            <p className="font-medium text-zinc-500 max-w-md mx-auto md:mx-0 text-sm md:text-base">
              Get in touch with our engineering team for free site feasibility study and load estimates.
            </p>
          </div>
          <Link
            to="/contact"
