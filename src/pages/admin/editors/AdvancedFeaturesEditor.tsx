  ...
  return (
    <div className="space-y-4 md:col-span-2">
      <div className="flex items-center justify-between">
        <label className="text-sm font-bold text-white block">Technical Features ({list.length} items)</label>
        <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">Fixed Specification Tags</span>
      </div>
        ...
        {list.map((item: string, idx: number) => (
          <div key={idx} className="flex items-center gap-3 bg-zinc-950 border border-zinc-900 p-3 rounded-2xl">
            ...
            <input
              ...
              placeholder="e.g. Mono PERC Solar Panels"
            />
          </div>
        ))}
      </div>
