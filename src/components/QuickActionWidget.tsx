...
const QuickActionWidget: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 pointer-events-auto">
      ...
      >
        {/* Pulsing Glow Aura behind button only */}
        <span className="absolute inset-0 rounded-full bg-[#25D366] blur-md opacity-60 animate-pulse -z-10" />
        <svg 
          viewBox="0 0 24 24" 
          className="w-6 h-6 fill-current text-white relative z-10"
        >
      ...
      >
        {/* Pulsing Gold Glow Aura behind button only */}
        <span className="absolute inset-0 rounded-full bg-yellow-400 blur-md opacity-40 animate-pulse -z-10" />
